...
public class RunPhymlMPJ extends RunPhyml 
{
	...
	public RunPhymlMPJ(Observer progress, ModelTest modelTest, Model[] models) 
	{
	...
	public void distribute() 
	{
		...
		Distributor distributor = new Distributor(modelList, this, ModelTest.MPJ_ME, ModelTest.MPJ_SIZE);
	...
	public void request() 
	{
		...
		while (true) 
		{
			...
			{
				Request modelRequest = MPI.COMM_WORLD.Isend(lastComputedModel, 0, 1, MPI.OBJECT, 0, Distributor.TAG_SEND_REQUEST);
				// prepare reception
				modelToReceive = new Model[1];
				// wait for request
				modelRequest.Wait();
				// receive model
				Request modelReceive = MPI.COMM_WORLD.Irecv(modelToReceive, 0, 1, MPI.OBJECT, 0, Distributor.TAG_SEND_MODEL);
				modelReceive.Wait();
	...
	public void execute() 
	{
		if (ModelTest.MPJ_ME == 0) 
		{
			...
			System.out.println("computing likelihood scores for " + models.length + " models with Phyml " + PHYML_VERSION);
		}
