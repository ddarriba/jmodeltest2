...
public class GuidedSearchManager {
	...
	public GuidedSearchManager(double guidedSearchThreshold, Model gtrModel, boolean filterFrequencies, boolean filterRateMatrix, boolean filterRateVariation, ModelTest modelTest) 
	{
	...
	public Model[] filterModels(Model[] models) 
	{
        ...
        for (Model model : models) 
        {
            ...
            for (int i=0; i<5; i++) 
            {
                for (int j=i+1; j<6; j++) 
                {
                    if (checkRates(model.getPartition(), i, j) && ratesFilter[rateIndex]) 
                    {
                        included = false;
                    }
                    rateIndex++;
	...
	public static Model[] getModelsSubset(Model[] models, String partition, int k, ModelTest modelTest) 
	{
        ArrayList<Model> modelsArray = new ArrayList<Model>();
	...
	private void setUpFilter(double guidedSearchThreshold, Model gtrModel) 
	{
        double threshold = adjustThreshold(guidedSearchThreshold, gtrModel.getLnL());
        ...
        for (int i = 0; i < (RATES - 1); i++) 
        {
            ...
            {
                rate[ratePairIndex] = computeRates(normalizedRates[i], normalizedRates[j]);
                ratePairIndex++;
            }
        }
        ...
        {
            if (computeFreqs(new double[] { gtrModel.getfA(), gtrModel.getfC(), gtrModel.getfG(), gtrModel.getfT() }) < (1 - threshold)) 
            {
        ...
        gammaFilter = doFilterRateVariation && gtrModel.getShape() > MIN_GAMMA_FILTER;
        invFilter = doFilterRateVariation && (gtrModel.getPinv() < MAX_INV_FILTER && gtrModel.getShape() > MIN_GAMMA_INV_FILTER);
        gammaInvFilter = doFilterRateVariation && (gtrModel.getShape() > MIN_GAMMA_FILTER && gtrModel.getPinv() < MAX_INV_FILTER);
        if (invFilter) 
        {
