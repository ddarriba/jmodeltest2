...
public class TreeSummary 
{
	...
	private List<Tree>	topologiesAIC = null, 
						...
						topologiesBIC = null,
						topologiesDT = null,
						sortedTopologies = null;
	
	public TreeSummary(Tree bestAIC, Tree bestAICc, Tree bestBIC, Tree bestDT, Model[] models) 
	{
		if (bestAIC != null) 
		{
		...
		void computeEuclideanDistances() 
		{
			...
			for (int i = 0; i < IC_COUNT; i++) 
			{
				if (bestTree[i] != null) 
				{
					...
					for (Model model : models) {
						double distance = euclideanDistances.getDistance(model.getTree(), bestTree[i]);
	...
	{
		int MAX_LEN = 80;
		if (topologiesAIC != null || topologiesBIC != null || topologiesAICc != null || topologiesDT != null) 
		{
			List<Tree> sortedTopologies = topologiesAIC != null ? topologiesAIC
					: topologiesBIC != null ? topologiesBIC : topologiesAICc != null ? topologiesAICc : topologiesDT;
			stream.println("::Optimized Topologies Summary::");
			stream.println("");
			stream.println("There are " + getNumberOfTopologies() + " different topologies.");
			stream.println("");
			for (int i = 0; i < getNumberOfTopologies(); i++) 
			{
				...
				int aicIndex = topologiesAIC != null ? aicIndexOf(currentTree) + 1 : -1;
				int aiccIndex = topologiesAICc != null ? aiccIndexOf(currentTree) + 1 : -1;
				int bicIndex = topologiesBIC != null ? bicIndexOf(currentTree) + 1 : -1;
				int dtIndex = topologiesDT != null ? dtIndexOf(currentTree) + 1 : -1;
				...
				stream.println("Models supporting:   " + summaryRow.models.size());
	...
	private String getIcRow(String name, int rankIndex, SummaryRow summaryRow, int criterionIndex) 
	{
		return name + "\t"
				...
				+ "\t"
				+ Utilities.asPercent(summaryRow.support[criterionIndex] * 100)
