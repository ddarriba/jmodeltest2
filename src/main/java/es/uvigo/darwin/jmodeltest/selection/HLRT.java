...
public class HLRT
{
 ...
 /**************************** compute *********************************
 *																		*
 *	Builds and tests the hierarchy of LRTs accordingly to the starting	*
 *	model and adding or removing parameters in a particular order		*
 *																		*
 ***********************************************************************/
	...
	public void compute (boolean forward, double alpha, boolean writePAUPblock)
		...
		for (i=0; i<hypotheses.length; i++)
			...
			if (competingModel == null)
				...
				hypotheses[i] + " from " + currentModel.getName() + ".\nRevise the order of hypotheses selected for the LRTs." +
				"\n...Skipping to the next hypothesis");
	//System.err.println("hypothesis = " +   hypotheses[i] + " -- not found best competing model");
				continue;
				}
	...
	//System.err.println("hypothesis = " +   hypotheses[i] + " -- found best competing model = " + competingModel.name);
			...
			if (hypotheses[i].equals("gamma") || hypotheses[i].equals("pinv"))
 ...
 /**************************** computeDynamical ************************
 *																		*
 *	Builds and tests the hierarchy of LRTs in a dynamic fashion
 *	 -- finding the best step to take (biggest increase in lnL) --		*
 *	 and adding or removing parameters in a particular order			*
 *																		*
 ***********************************************************************/

	public void computeDynamical (boolean forward, double alpha, boolean writePAUPblock)
		{
		...
		while (thereAreValidHypotheses)
			...
			for (i=0; i<hypotheses.length; i++)
				...
				if (competingModel != null)
					...
					if ((forward && lnDifference > bestLnDifference) || (!forward && lnDifference < bestLnDifference))
						{
			...
			if (bestCompetingModel == null)
				{
				stream.print("\nNo best competing model for any of the " + hypotheses.length 
				...
				stream.println (", given the current model = " + currentModel.getName());
			//System.err.println("hypothesis = " +   hypotheses[bestHypothesisIndex] + " -- not found best competing model");
				thereAreValidHypotheses = false;
				continue;
				}
			
		/*	stream.println("\nBest hypothesis = " + hypotheses[bestHypothesisIndex] +
				"\n current model = " + currentModel.name +
				"\n competing model = " + bestCompetingModel.name + 
				"\n best lnL increase = " +  bestLnDifference);*/
				
			//System.err.println("hypothesis = " +   hypotheses[bestHypothesisIndex] + " -- found best competing model = " + bestCompetingModel.name);
			...
			stream.println("\nTesting " + hypotheses[bestHypothesisIndex] + " hypothesis");
			stream.printf("Null model        = %-8s", nullModel.getName());
			stream.printf("\t-lnL = %6.4f", nullModel.getLnL());
			stream.printf("\nAlternative model = %-8s", altModel.getName());
			stream.printf("\t-lnL = %6.4f", altModel.getLnL());
			...
			if (hypotheses[bestHypothesisIndex].equals("gamma") || hypotheses[bestHypothesisIndex].equals("pinv"))
				P = LRTboundary(nullModel, altModel);
			else
				P = LRT(nullModel, altModel);
 ...
 /**************************** findCompetingModel***********************
 *																		*
 *	Finds the corresponding competing model given the current    	    *
 *	model and hypothesis that is being tested. The null model			*
 *	has to be nested within the alternative, and they can only differ	*
 ...
 ***********************************************************************/
	...
	public Model findCompetingModel (Model current, String test, boolean adding)
		...
		for (Model model : ModelTest.model)
			...
			if ((!current.ispT() && competing.ispT()) || (!competing.ispT() && current.ispT()))
				{
				if (test.equals("titv"))
					isTest = true;
				if (!current.ispR() && !competing.ispR()) // because we will increase this distance later for models with pR
					distance++;
				}
			...
			else if (((adding) && (current.ispT() || current.ispR())) || ((!adding) && (competing.ispT() || competing.ispR())))
				{
			...
			//System.err.println("test = " + test + "   current = " + current.name  + "  competing = " + competing.name +
...
/**************************** BestTIMTPM *******************************
 *																		  *
 *	Returns the corresponding individual model within the families		  *
 *	TPM, TPMuf, TIM and TIMef											  *
 *																		  *
 *************************************************************************/
	...
	public Model BestTIMTPM (Model whichModel)
		...
		//System.err.println ("Find best among " + model1 + " and " + model2 + " and " + model3);
...
/**************************** findBest *******************************
 *																		*
 *	Finds the model with the highest likelihood out of three		    *					
 *	  TIM or TPM given models											*	
 *																		*
 ************************************************************************/

	public Model findBest (String modelName1, String modelName2, String modelName3)
		{
		Model m1, m2, m3;
 ...
 /**************************** LRT *************************************
 *																		*
 *	Computes a likelihood ratio test between two model and returns   	*					
 *	  a P-value according to a standard chi2 distribution				*	
 *																		*
 ***********************************************************************/
 ...
 *																		*
 *	Computes a likelihood ratio test between two model and returns   	*					
 *	  a P-value according to a mixed chi2 distribution					*	
 *																		*
 ***********************************************************************/
