...
public class HLRT
{
    ...
    public void computeDynamical(boolean forward, double alpha, boolean writePAUPblock) 
    {
        ...
        while (thereAreValidHypotheses) 
        {
	        ...
	        {
	            stream.print("\nNo best competing model for any of the " + hypotheses.length + " remaining hypotheses: ");
	            for (i = 0; i < hypotheses.length; i++) 
	            {
	                stream.print(hypotheses[i]);
	                if (i < hypotheses.length - 1)
    ...
    private double LRTboundary(Model model0, Model model1) 
    {
        ...
        else 
        {
            if (df == 1)
            	...
            	prob = (Statistics.chiSquareProbability(delta, df - 1) + Statistics.chiSquareProbability(delta, df)) / 2;
        }
